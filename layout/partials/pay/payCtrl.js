angular.module('btw').controller('payCtrl', function ($scope, $rootScope, $filter, apiService, ipCookie, $http, $window, userService, postSerivice, $translate) {
    ...
    $scope.calculateFee = function () {
        if ($scope.amount && Number($scope.amount) > 0) {
            ...
            var fee = BtwJS.transaction.calculateFee(amount);
    ...
    $scope.sentMsg = function () {
        ...
        if(!$rootScope.currencyName){
            ...
            //transaction = BtwJS.transaction.createTransaction(String($scope.fromto), amount, message, userService.secret, $scope.secondPassword);
        } else {
            amount = ($scope.amount*Math.pow(10, $rootScope.precision)).toFixed(0);
            //transaction = BtwJS.uia.createTransfer(String($rootScope.currencyName), amount, String($scope.fromto), message, userService.secret, $scope.secondPassword)
        }
