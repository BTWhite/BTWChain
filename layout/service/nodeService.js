angular.module('btw').service('nodeService', function ($http) {

    let CHECK_STATUS_INTERVAL = 1000 * 30;
    let CHECK_SERVER_TIMEOUT = 1000 * 3;
    let MAX_RESPONSE_TIME = 1000 *3;
    let MAX_SERVER_LOAD = 0.8;
    let MAX_BLOCK_BEHINDS = 3;
    let SERVER_STATE_OK = 2;
    let MAX_CANDIDATE_SERVERS = 10;
    let MAX_FIND_DEPTH = 5;
    let HTTP_STATUS_OK = 200;

    function BtwServer(url){
        this.timer = null;
        ...
        this.isServerAvalible = function(ignorePending){
            return ( this.state=="success" || (ignorePending && this.state == "pending") ) && 
                this.versionNotLessThan("1.0.4") && this.isHealthy();
        };
    ...
    BtwServer.compareServer = function(server1, server2){
    ...
    function registerServer(serverUrl){
        if (findServerIndex(serverUrl) < 0){
            var server = new BtwServer(serverUrl);
    ...
    function sortServers(serverArray){
        serverArray = serverArray || servers;
        return serverArray.sort(BtwServer.compareServer);
    }
    ...
    function adjustClientDriftSeconds(server){
        ...
        BtwJS.options.set('clientDriftSeconds', timeAdjust);
    }
