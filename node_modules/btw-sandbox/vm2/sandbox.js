	...
	const _prepareRequire = function (current_dirname) {
		const _require = function (modulename) {
			...
			if (vm.options.require.root) {
				let allowed = false
				for (let i in vm.options.require.root) {
					let requiredPath = pa.resolve(vm.options.require.root[i]);
					if (dirname.indexOf(requiredPath) === 0) {
						allowed = true
						break
					}
				}
				if (!allowed) throw new VMError(`Module '${modulename}' is not allowed to be required. The path is outside the border!`, "EDENIED");
			}
