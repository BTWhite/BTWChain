...
__cur.attachApi = function () {
    ...
    if (process.env.DEBUG && process.env.DEBUG.toUpperCase() == "TRUE") {
        router.get('/getAllAccounts', function (req, res) {
            return res.json({success: true, accounts: __cur.accounts});
        });
    }
...
__cur.openAccount2 = function (publicKey, cb) {
    var address = self.generateAddressByPublicKey2(publicKey);
    self.getAccount({address: address}, function (err, account) {
        if (err) return cb(err);
        var acc = account || {
...
shared.open = function (req, cb) {
    ...
    library.scheme.validate(body, {
        ...
        __cur.openAccount(body.secret, function (err, account) {
...
shared.open2 = function (req, cb) {
    ...
    library.scheme.validate(body, {
        ...
        __cur.openAccount2(body.publicKey, function (err, account) {
            var accountData = null;
            if (!err) {
                accountData = {
                    address: account.address,
