...
__cur.unconfirmedNumber = 0;
__cur.unconfirmedTransactions = [];
__cur.unconfirmedTransactionsIdIndex = {};
...
__cur.attachApi = function () {
    ...
    __cur.attachStorageApi();
};

__cur.attachStorageApi = function () {
    var router = new Router();
...
__cur.addUnconfirmedTransaction = function (transaction, sender, cb) {
    self.applyUnconfirmed(transaction, sender, function (err) {
        ...
        __cur.unconfirmedTransactions.push(transaction);
        __cur.unconfirmedTransactionsIdIndex[transaction.id] = __cur.unconfirmedTransactions.length - 1;
        __cur.unconfirmedNumber++;
...
Transactions.prototype.getUnconfirmedTransaction = function (id) {
    var index = __cur.unconfirmedTransactionsIdIndex[id];
    return __cur.unconfirmedTransactions[index];
};

Transactions.prototype.getUnconfirmedTransactionList = function (reverse, limit) {
    var a = [];

    for (var i = 0; i < __cur.unconfirmedTransactions.length; i++) {
        if (__cur.unconfirmedTransactions[i] !== false) {
            a.push(__cur.unconfirmedTransactions[i]);
        }
    }
...
Transactions.prototype.removeUnconfirmedTransaction = function (id) {
    if (__cur.unconfirmedTransactionsIdIndex[id] == undefined) {
        return
    }
    var index = __cur.unconfirmedTransactionsIdIndex[id];
    delete __cur.unconfirmedTransactionsIdIndex[id];
    __cur.unconfirmedTransactions[index] = false;
    __cur.unconfirmedNumber--;
};

Transactions.prototype.hasUnconfirmedTransaction = function (transaction) {
    var index = __cur.unconfirmedTransactionsIdIndex[transaction.id];
    return index !== undefined && __cur.unconfirmedTransactions[index] !== false;
};
...
Transactions.prototype.processUnconfirmedTransaction = function (transaction, broadcast, cb) {
    ...
    if (__cur.unconfirmedTransactionsIdIndex[transaction.id] !== undefined) {
    ...
    modules.accounts.setAccountAndGet({publicKey: transaction.senderPublicKey}, function (err, sender) {
        function done(err) {
            ...
            __cur.addUnconfirmedTransaction(transaction, sender, function (err) {
                if (err) {
                    return cb(err);
                }
...
Transactions.prototype.undoUnconfirmedList = function (cb) {
    var ids = [];
    async.eachSeries(__cur.unconfirmedTransactions, function (transaction, cb) {
...
Transactions.prototype.receiveTransactions = function (transactions, cb) {
    if (__cur.unconfirmedNumber > constants.maxTxsPerBlock) {
...
shared.getTransaction = function (req, cb) {
    ...
    library.scheme.validate(query, {
        ...
        __cur.getById(query.id, function (err, transaction) {
            if (!transaction || err) {
