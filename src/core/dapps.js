...
const Sandbox = require('btw-sandbox');
const rmdir = require('rimraf');
const ip = require('ip');
const valid_url = require('valid-url');
const DecompressZip = require('decompress-zip');
const dappCategory = require('../utils/dapp-category.js');
...
__cur.loading = {};
__cur.removing = {};
__cur.unconfirmedNames = {};
__cur.unconfirmedLinks = {};
__cur.unconfirmedAscii = {};
__cur.appPath = '';
__cur.dappsPath = '';
__cur.sandboxes = {};
__cur.dappready = {};
__cur.routes = {};
__cur.unconfirmedOutTansfers = {};
__cur.defaultRouteId = null;

const WITNESS_CLUB_DAPP_NAME = 'btw-community';
...
function OutTransfer() {
    ...
    this.process = function (trs, sender, cb) {
        ...
        library.model.getDAppById(transfer.dappId, function (err, dapp) {
            ...
            if (__cur.unconfirmedOutTansfers[trs.asset.outTransfer.transactionId]) {
    ...
    this.apply = function (trs, block, sender, cb) {
        var transfer = trs.asset.outTransfer;
        __cur.unconfirmedOutTansfers[transfer.transactionId] = false;
    ...
    this.undo = function (trs, block, sender, cb) {
        __cur.unconfirmedOutTansfers[transfer.transactionId] = true;
    ...
    this.getBytes = function (trs) {
        try {
            ...
            if (trs.asset.inTransfer.currency !== 'XAS') {
                currency = new Buffer(trs.asset.inTransfer.currency, 'utf8');
...
function DApp() {
    ...
    this.applyUnconfirmed = function (trs, sender, cb) {
        if (__cur.unconfirmedNames[trs.asset.dapp.name]) {
            return setImmediate(cb, "Dapp name already exists");
        }

        if (trs.asset.dapp.link && __cur.unconfirmedLinks[trs.asset.dapp.link]) {
            return setImmediate(cb, "Dapp link already exists");
        }

        __cur.unconfirmedNames[trs.asset.dapp.name] = true;
        __cur.unconfirmedLinks[trs.asset.dapp.link] = true;

        setImmediate(cb)
    };

    this.undoUnconfirmed = function (trs, sender, cb) {
        delete __cur.unconfirmedNames[trs.asset.dapp.name];
        delete __cur.unconfirmedLinks[trs.asset.dapp.link];
...
function DApps(cb, scope) {
    ...
    __cur.appPath = library.config.baseDir;
    __cur.dappsPath = library.config.dappsDir;
    ...
    fs.exists(path.join(library.config.publicDir, 'dapps'), function (exists) {
        if (exists) {
            rmdir(path.join(library.config.publicDir, 'dapps'), function (err) {
                ...
                __cur.createBasePathes(function (err) {
...
__cur.attachApi = function () {
    ...
    router.get('/', function (req, res, next) {
        req.sanitize(req.query, {
            ...
            __cur.count(function (err, count) {
                if (err) {
                    return res.json({success: false, error: "Failed to count dapps"});
                }
                __cur.list(query, function (err, dapps) {
                    if (err) {
    ...
    router.get('/get', function (req, res, next) {
        req.sanitize(req.query, {
            ...
            __cur.get(query.id, function (err, dapp) {
                if (err) {
                                    ...
                                    library.dbLite.query("SELECT transactionId, name, description, tags, link, type, category, icon FROM dapps WHERE rowid IN (" + rowids.join(',') + ")" + categorySql, {category: category}, {
                                        ...
                                        if (err) {
                                            ...
                                            if (query.installed === null || typeof query.installed === 'undefined') {
                                                ...
                                                __cur.getInstalledIds(function (err, installed) {
                                                    if (err) {
                                                        return res.json({
                                                            success: false,
    ...
    router.post('/install', function (req, res, next) {
        req.sanitize(req.body, {
            ...
            __cur.get(body.id, function (err, dapp) {
                if (err) {
                    return res.json({success: false, error: err});
                }

                __cur.getInstalledIds(function (err, ids) {
                    if (err) {
                        return res.json({success: false, error: err});
                    }
                    ...
                    if (__cur.removing[body.id] || __cur.loading[body.id]) {
                        return res.json({success: false, error: "This DApp already on downloading/removing"});
                    }

                    __cur.loading[body.id] = true;

                    __cur.installDApp(dapp, function (err, dappPath) {
                        if (err) {
                            __cur.loading[body.id] = false;
                            return res.json({success: false, error: err});
                        } else {
                            if (dapp.type == 0) {
    ...
    router.get('/installed', function (req, res, next) {
        __cur.getInstalledIds(function (err, files) {
            if (err) {
                library.logger.error(err);
            ...
            __cur.getByIds(files, function (err, dapps) {
                if (err) {
                    library.logger.error(err);
    ...
    router.post('/uninstall', function (req, res, next) {
        req.sanitize(req.body, {
            ...
            __cur.get(body.id, function (err, dapp) {
                ...
                __cur.removing[body.id] = true;

                if (__cur.launched[body.id]) {
                    // Stop dapp first
                    __cur.stop(dapp, function (err) {
                        if (err) {
                            library.logger.error(err);
                            return res.json({success: false, error: "Can't stop dapp, check logs"});
                        } else {
                            __cur.launched[body.id] = false;
                            __cur.removeDApp(dapp, function (err) {
                                __cur.removing[body.id] = false;
    ...
    router.post('/launch', function (req, res, next) {
        ...
        __cur.launch(req.body, function (err) {
            if (err) {
                return res.json({"success": false, "error": err});
            }
...
// Private methods
__cur.get = function (id, cb) {
    library.model.getDAppById(id, cb)
};

__cur.getByIds = function (ids, cb) {
    library.model.getDAppsByIds(ids, cb)
};

__cur.getDAppBalance = function (req, cb) {
...
__cur.getDAppBalances = function (req, cb) {
    if (!req.params) return cb('Invalid parameters');
    if (!req.params.dappId || req.params.dappId.length > 64) return cb('Invalid dapp id');
    let filter = {
...
__cur.createBasePathes = function (cb) {
    async.series([
        // function (cb) {
        ...
        // },
        function (cb) {
            fs.exists(__cur.dappsPath, function (exists) {
                if (exists) {
                    return setImmediate(cb);
                } else {
                    fs.mkdir(__cur.dappsPath, cb);
                }
            });
        },
        function (cb) {
...
// __cur.installDependencies = function (dapp, cb) {
//   var dappPath = path.join(__cur.dappsPath, dapp.transactionId);
...
__cur.getInstalledIds = function (cb) {
    fs.readdir(__cur.dappsPath, function (err, files) {
        if (err) {
            return setImmediate(cb, err);
        }
...
__cur.removeDApp = function (dapp, cb) {
    var dappPath = path.join(__cur.dappsPath, dapp.transactionId);
    ...
    async.waterfall([
        ...
        async.apply(__cur.readJson, path.join(dappPath, "blockchain.json")),
...
__cur.downloadLink = function (dapp, dappPath, cb) {
    var tmpDir = "tmp";
    var tmpPath = path.join(__cur.appPath, tmpDir, dapp.transactionId + ".zip");
...
__cur.installDApp = function (dapp, cb) {
    var dappPath = path.join(__cur.dappsPath, dapp.transactionId);
    ...
    async.series({
            ...
            performInstall: function (serialCb) {
                return __cur.downloadLink(dapp, dappPath, serialCb);
            }
        },
        function (err) {
            if (err) {
...
__cur.symlink = function (dapp, cb) {
    var dappPath = path.join(__cur.dappsPath, dapp.transactionId);
    var dappPublicPath = path.resolve(dappPath, "public");
    var dappPublicLink = path.resolve(__cur.appPath, "public", "dist", "dapps", dapp.transactionId);
...
__cur.apiHandler = function (message, callback) {
    // Get all modules
    try {
...
__cur.dappRoutes = function (dapp, cb) {
    var dappPath = path.join(__cur.dappsPath, dapp.transactionId);
    var dappRoutesPath = path.join(dappPath, "routes.json");

    var routes = Sandbox.routes;

    __cur.routes[dapp.transactionId] = new Router();

    routes.forEach(function (router) {
        if (router.method == "get" || router.method == "post" || router.method == "put") {
            __cur.routes[dapp.transactionId][router.method](router.path, function (req, res) {
                var reqParams = {
    ...
    if (!__cur.defaultRouteId) {
        __cur.defaultRouteId = dapp.transactionId;
        library.network.app.use('/api/dapps/default/', __cur.routes[dapp.transactionId]);
    }
    library.network.app.use('/api/dapps/' + dapp.transactionId + '/', __cur.routes[dapp.transactionId]);
    library.network.app.use(function (err, req, res, next) {
        if (!err) return next();
        library.logger.error(req.url, err.toString());
        res.status(500).send({success: false, error: err.toString()});
...
__cur.launch = function (body, cb) {
    library.scheme.validate(body, {
        ...
        async.auto({
            dapp: async.apply(__cur.get, body.id),

            installedIds: async.apply(__cur.getInstalledIds),

            symlink: ['dapp', 'installedIds', function (next, results) {
                if (results.installedIds.indexOf(body.id) < 0) {
                    return next('Dapp not installed');
                }
                __cur.symlink(results.dapp, next);
            }],

            launch: ['symlink', function (next, results) {
                __cur.launchApp(results.dapp, body.params, next);
            }],

            route: ['launch', function (next, results) {
                __cur.dappRoutes(results.dapp, function (err) {
                    if (err) {
                        return __cur.stop(results.dapp, next);
                    }
                    next();
                });
            }]
...
__cur.readJson = function (file, cb) {
...
__cur.launchApp = function (dapp, params, cb) {
    var dappPath = path.join(__cur.dappsPath, dapp.transactionId);

    __cur.readJson(path.join(dappPath, "config.json"), function (err, dappConfig) {
        if (err) {
        ...
        async.eachSeries(dappConfig.peers, function (peer, cb) {
            ...
            var sandbox = new Sandbox(dappPath, dapp.transactionId, params, __cur.apiHandler, true, library.logger);
            __cur.sandboxes[dapp.transactionId] = sandbox;
...
__cur.stop = function (dapp, cb) {
    var dappPublicLink = path.join(__cur.appPath, "public", "dapps", dapp.transactionId);
    ...
    async.series([
        ...
        function (cb) {
            if (__cur.sandboxes[dapp.transactionId]) {
                __cur.sandboxes[dapp.transactionId].exit();
            }

            delete __cur.sandboxes[dapp.transactionId];

            setImmediate(cb)
        },
        function (cb) {
            delete __cur.routes[dapp.transactionId];
            setImmediate(cb);
        }
    ], function (err) {
        return setImmediate(cb, err);
...
__cur.addTransactions = function (req, cb) {
    var body = req.body;
    library.scheme.validate(body, {
        type: "object",
        properties: {
...
DApps.prototype.request = function (dappId, method, path, query, cb) {
    if (!__cur.sandboxes[dappId]) {
        return cb("Dapp not found");
    }
    if (!__cur.dappready[dappId]) {
        return cb("Dapp not ready");
    }
    __cur.sandboxes[dappId].sendMessage({
        method: method,
        path: path,
        query: query
    }, cb);
...
DApps.prototype.cleanup = function (cb) {
    var keys = Object.keys(__cur.launched);

    async.eachSeries(keys, function (id, cb) {
        if (!__cur.launched[id]) {
            return setImmediate(cb);
        }
        __cur.stop({
            transactionId: id
...
DApps.prototype.onBlockchainReady = function () {
    __cur.getInstalledIds(function (err, dappIds) {
        if (err) {
        ...
        async.eachSeries(dappIds, function (id, next) {
            ...
            __cur.launch({id: id, params: dappParams}, function (err) {
                if (err) {
            ...
            library.model.getDAppByName(WITNESS_CLUB_DAPP_NAME, function (err, clubInfo) {
                if (err) {
                    library.logger.error('Failed to query btw witness club', err)
                } else if (!clubInfo) {
                    library.logger.warn('Btw witness club dapp is not found')
                } else {
...
DApps.prototype.onDeleteBlocksBefore = function (block) {
    Object.keys(__cur.sandboxes).forEach(function (dappId) {
        let req = {
            query: {
                topic: "rollback",
...
shared.setReady = function (req, cb) {
    __cur.dappready[req.dappId] = true;
...
shared.registerInterface = function (req, cb) {
    ...
    __cur.routes[dappId][method](path, function (req, res) {
        var reqParams = {
