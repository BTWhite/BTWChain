...
__cur.attachApi = function () {
    ...
    router.post('/forging/enable', function (req, res) {
        ...
        library.scheme.validate(body, {
            ...
            if (__cur.keypairs[keypair.publicKey.toString('hex')]) {
            ...
            modules.accounts.getAccount({publicKey: keypair.publicKey.toString('hex')}, function (err, account) {
                ...
                if (account && account.isDelegate) {
                    __cur.keypairs[keypair.publicKey.toString('hex')] = keypair;
    ...
    router.post('/forging/disable', function (req, res) {
        ...
        library.scheme.validate(body, {
            ...
            modules.accounts.getAccount({publicKey: keypair.publicKey.toString('hex')}, function (err, account) {
                ...
                if (account && account.isDelegate) {
                    delete __cur.keypairs[keypair.publicKey.toString('hex')];
    ...
    router.get('/forging/status', function (req, res) {
        ...
        library.scheme.validate(query, {
            ...
            return res.json({success: true, enabled: !!__cur.keypairs[query.publicKey]});
        });
    });
...
__cur.getKeysSortByVote = function (cb) {
...
__cur.getBlockSlotData = function (slot, height, cb) {
    self.generateDelegateList(height, function (err, activeDelegates) {
        ...
        for (; currentSlot < lastSlot; currentSlot += 1) {
            ...
            if (delegate_id && __cur.keypairs[delegate_id]) {
                return cb(null, {time: slots.getSlotTime(currentSlot), keypair: __cur.keypairs[delegate_id]});
            }
        }
        cb(null, null);
    });
...
__cur.loop = function (cb) {
    if (!__cur.forgingEanbled) {
        library.logger.trace('Loop:', 'forging disabled');
        return setImmediate(cb);
    }
    if (!Object.keys(__cur.keypairs).length) {
        library.logger.trace('Loop:', 'no delegates');
        return setImmediate(cb);
    }

    if (!__cur.loaded || modules.loader.syncing() || !modules.round.loaded()) {
    ...
    //libary.logger.trace('Loop:', Date.now() % 10000);
    if (Date.now() % 1000 > 5000) {
        library.logger.trace('Loop:', 'maybe too late to collect votes');
        return setImmediate(cb);
    }

    __cur.getBlockSlotData(currentSlot, lastBlock.height + 1, function (err, currentBlockData) {
...
__cur.loadMyDelegates = function (cb) {
    var secrets = null;
...
Delegates.prototype.getActiveDelegateKeypairs = function (height, cb) {
    self.generateDelegateList(height, function (err, delegates) {
        ...
        var results = [];
        for (var key in __cur.keypairs) {
            if (delegates.indexOf(key) !== -1) {
                results.push(__cur.keypairs[key]);
            }
        }
        cb(null, results);
    });
...
Delegates.prototype.generateDelegateList = function (height, cb) {
    __cur.getKeysSortByVote(function (err, truncDelegateList) {
        if (err) {
            return cb(err);
        }
...
Delegates.prototype.onBlockchainReady = function () {
    ...
    __cur.loadMyDelegates(function nextLoop(err) {
        if (err) {
